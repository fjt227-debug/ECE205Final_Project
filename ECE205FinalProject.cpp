...
Node* constructTree(string postfix) {
    ...
    Node* t, * t1, * t2; //Temporary node pointers
    for (int i = 0; i < postfix.length(); i++) {
        if (postfix[i] == ' ') continue; //Skip spaces

        if (isOperator(postfix[i])) {
            t = new Node((int)postfix[i], true); //Create operator node

            t1 = st.top(); //First popped node becomes right child
            st.pop();
            t2 = st.top(); //Second popped node becomes left child
            st.pop();

            t->right = t1;
            t->left = t2;

            st.push(t); //Push the new subtree onto the stack
        }
        else {
            int num = 0;
            ...
            i--;
            t = new Node(num, false); //Create operand node
            st.push(t);
            }
        }
        t = st.top(); //Last node on stack is the root
        st.pop();
        return t;
}
...
int evaluatePostfix(string postfix) {
    ...
    for (int i = 0; i < postfix.length(); i++) {
        ...
        if (isOperator(postfix[i])) {
            int op2 = S.top(); S.pop(); //Pop right operand
            int op1 = S.top(); S.pop(); //Pop left operand
            int res = 0;
            ...
            case '*': res = op1 * op2; break;
            case '/':
                if (op2 == 0) throw runtime_error("Division by zero."); //Prevents division by zero
                res = op1 / op2;
                break;
            }
            S.push(res); //Push result back onto stack
        }
        else {
            int num = 0;
            while (i < postfix.length() && postfix[i] >= '0' && postfix[i] <= '9') {
            ...
            i--;
            S.push(num); //Push operand onto stack 
        }
    }
    return S.top(); //Final value on the stack
}
